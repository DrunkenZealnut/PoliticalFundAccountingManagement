...
- All dates stored as `YYYYMMDD` strings (not DATE type). UI uses `YYYY-MM-DD`, convert on save/display. `acc_book.acc_time` (`scripts/014`, CHAR(4) "HHmm", NULL 허용) carries the intra-day 거래 시각. **거래(장부/목록) 정렬 SSOT는 `lib/accounting/acc-book-sort.ts`의 `compareAccDateTime`(`acc_date → acc_time`, 시각 미입력은 nulls first).** 신규 입력 행은 `acc_sort_num`이 NULL이라 같은 날 tie-break가 비결정적이므로, acc_book 거래를 날짜순 표시/정렬하는 새 경로는 반드시 이 SSOT(Supabase 경로는 `.order("acc_time", {ascending:true, nullsFirst:true})`)를 1차 키로 써야 한다 — 누락 시 같은 날 수입/지출이 입력순으로 뒤바뀐다(실제 버그였음). 잔액을 누계하는 장부는 동시각·미입력 시 `incm_sec_cd`로 수입 우선 tie-break(잔액 음수 방지). 영수증 채번(`receipt-no.ts`)·첨부서류 목록은 의도적으로 제외.
