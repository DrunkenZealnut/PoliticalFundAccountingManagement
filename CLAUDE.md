...
- **연도(선거주기) 분리 가드 (`year-data-separation`, v0.27.0.0)**: 거래일↔org 회계기간 검증 SSOT는 `lib/accounting/acc-period.ts`(`isAccDateInOrgPeriod` — `[pre_acc_from(이월) || acc_from] ~ acc_to`, YYYYMMDD 사전식). `api/acc-book`(insert/update/batch_insert)가 기간 밖이면 `OUT_OF_PERIOD`(400), `_allowOutOfPeriod`(단건)/`allowOutOfPeriod`(배치) override 시 통과. 클라는 `lib/acc-book-client.ts`의 `postAccBook`(OUT_OF_PERIOD·SETTLED_PERIOD→confirm→재요청, 최대 2회 순차) 경유(income·**expense**·document-register·batch-import). **지출(expense)의 acc_book insert/update 는 `postAccBook`(API) 경유로 통일됨**(program-wide-review BX1 — 서버가 익명 resolve·기간검증·소속검증 일괄 처리, 과거의 클라측 인라인 `isAccDateInOrgPeriod` 가드 제거). **단 expense 삭제(단건·일괄)는 아직 supabase 브라우저 클라 직접**이라 API 서버 가드가 안 닿는다(옛 주기 잠금은 클라 인라인 가드로 income·expense 삭제 핸들러에 별도 적용). org당 선거주기는 `acc_from` 연도(= `election_cycle`, `scripts/021`). 혼입 점검: `app/scripts/scan-year-contamination.mjs`(exit≠0=혼입). **UI 분리(`year-separation-p2`, v0.28.0.0)**: 주기 판정 SSOT가 `acc-period.ts`에 `currentCycleOf`(사용자 기관 중 최신=현 주기)·`isOldCycle`(orgCycle<currentCycle, 보수적 false)로 확장. 옛 주기 org는 `hooks/use-org-cycle-lock.ts`(`useOrgCycleLock` — 자기완결형, 사용자 전체 기관 acc_from 조회 후 현 org가 옛 주기면 `locked`, 잠금해제는 org 단위 기억) + `components/dashboard/OrgCycleLockBanner.tsx`로 입력 4페이지(income·expense·document-register·batch-import) 저장 핸들러 가드 + 배너(읽기전용), `dashboard/layout` 사이드바 자물쇠 표식, `select-organ` 주기 배지·현 주기 필터를 건다. 잠금은 **UX 가드**일 뿐 데이터 무결성 1차 방어는 위 P1 서버검증이 계속 담당. **FR-07(산출물 주기 외 거래 경고)**: SSOT `acc-period.ts countOutOfPeriodRows`로 export-sqlite(응답 헤더 `X-Out-Of-Period`)·backup 페이지 alert·결산 handleSettle alert에 회계기간 밖 거래 경고 적용(차단 아님, 은폐 금지). reports·HWPX 서식은 후속.
- Migrations live in `app/scripts/0NN_*.sql` and are applied **manually via the Supabase SQL editor** (DDL cannot run through the service-role REST client). Latest is `025` (`012` = `delete_org_data` RPC, `013` = `finalize_settlement` RPC — **`023`으로 본문 갱신됨**, `014` = `acc_book`/`acc_book_bak`에 `acc_time CHAR(4)` 컬럼 추가 — **`019`로 폐기됨, 적용 금지**; `015` = `acc_book`에 `claim_amt` 컬럼 추가 — 보전 최종청구액; `016`/`017` = 과목 배분 추적 컬럼·영구화; `019` = `acc_book`/`acc_book_bak`에서 `acc_time` 컬럼 DROP — 시분초 미사용(`acc-time-removal`); `020` = `opinion` 복수 감사자 컬럼; `021` = `organ.election_cycle` 추가 — 선거주기(연도) 분리(`year-data-separation`), `acc_from` 연도로 백필, additive; `022` = `customer`/`customer_addr` org별 RLS 격리 — 006의 `auth.uid() IS NOT NULL`을 `org_id IN (user_organ...)`로 좁힘(익명 org_id NULL은 공용 유지, customer_addr는 cust_id 조인). **브라우저 직접쿼리 방어선일 뿐 — API는 service_role로 RLS 우회하므로 API 계층 IDOR는 organ 라우트식 user_organ 소속검증을 별도 적용해야 닫힘**; `023` = `opinion.settled_at` 추가(결산확정 플래그) + `finalize_settlement` RPC 갱신 — **organ.acc_from/acc_to 덮어쓰기 제거**(BX8, 회계기간·주기 판정 SSOT 오염 방지)·결산기간은 opinion 에만 저장, 확정 시각 기록. **opinion 컬럼 추가라 export-sqlite `APP_ONLY_OPINION_COLUMNS`에 `settled_at` 등록 필수**[반영됨]; `024` = `customer` 공유 익명(`org_id IS NULL AND name='익명'`) 부분 유니크 인덱스 — 재중복 차단(B3, **중복 제거 후 적용**, `cleanup-anon-customers.mjs`); `025` = `org_income_expense_totals(p_org_id)` RPC — acc-book GET 요약합계 SUM GROUP BY(P-3, **코드에 전건 폴백 있어 미적용도 안전**)). When adding migrations that touch `acc_book`/`acc_book_bak`, audit the export-sqlite route (see SQLite gotchas below — app-only columns must be stripped).

### Source Layout (`app/src/`)

```
app/api/          → route groups: codes, customers, acc-book, organ, excel/export, system/{export-sqlite,import-sqlite,recompute-settlement,workflow-status}, address/search, evidence-file, reimbursement/claim-form/aggregate, hwpx/{generate,income-ledger,accounting-report,reimbursement-claim,reimbursement-doclist}
app/dashboard/    → 28 pages: income, expense, document-register (manual entry), income-expense-book (재조정 수입·지출부 뷰어), income-expense-report, reports, submit, reset, backup (SQLite backup/restore), customer, customer-batch, organ, aggregate, audit, forms, settlement, estate, codes, submission-forms (선관위 제출서류 HWPX 생성), submission-wizard (제출 산출물 마법사 — 준비상태 점검·미리보기·zip 일괄생성, submission-forms/reports/reimbursement 오케스트레이션), etc. Several are org-type-gated (party-summary, supporter-summary, support-detail, donors) — see Org-Type Differentiation below.
...
components/wizard/ → 제출 마법사 4스텝(TrackSelectStep, ReadinessStep, PreviewStep, GenerateStep) — 판정·수치는 lib/accounting/submission-readiness·lib/submission 순수 함수를 그대로 표시만
hooks/            → use-code-values, use-donation-limit, use-sort, use-undo, use-hwpx-prefill, use-org-cycle-lock (옛 선거주기 org 읽기전용 잠금 — year-separation-p2)
lib/supabase/     → client.ts (browser), server.ts (SSR), middleware.ts (session)
lib/chat/         → faq-data.ts (static FAQ items only)
lib/accounting/   → Business logic: settlement-calc (balances), funding-source, submission-forms, submission-readiness (제출 마법사 신호등 판정 — 기존 SSOT 조합만, 신규 계산 0), + PFund2 SQLite compat (organ-pair, pfund2-constants, parity-errors, import-helpers)
...
lib/excel-template/ → Excel 산출물 빌더(ExcelJS 직접): burden-cost-form(부담비용), reimbursement-claim-form(보전청구서), income-expense-book(수입·지출부 13컬럼), report-combos(reports 페이지 계정×과목 조합), build-report-workbook(reports 배치 Excel 워크북 빌더 — reports/page.tsx와 submission-wizard GenerateStep이 공유). (구 선관위 템플릿 기반 경로 `/api/excel/report`+data-query/mappings/index 등은 v0.18.0.0에서 데드코드로 제거)
lib/submission/   → 제출 마법사 오케스트레이션 헬퍼(순수): wizard-forms(orgType별 트랙 서식 집합), generate-form(HWPX 생성 payload·엔드포인트 매핑 — FormInputPanel과 공유)
...
### 재배분(Reallocation) SSOT — `buildLedgerRows` Pass 파이프라인

후보자(82~85) 거래는 원본 `acc_book`을 **불변**으로 두고 **보고 시점에만** (계정×과목)을 재조정한다. 이 재배분이 화면·Excel·HWPX·결산·.db 모든 산출물의 단일 진실원이며, 코어는 `lib/accounting/ledger-allocation.ts`의 순수 함수 `buildLedgerRows` = **Pass0→L→1→2→3→4** (v0.31.0.0):

| Pass | 파일 : export | 역할 |
|------|---------------|------|
| Pass0 | `adjust-negative-income.ts` : `adjustNegativeIncome` | 음수 수입(incm=1 ∧ acc_amt<0) → 양수 지출(incm=2) 정규화(선관위 정합) |
| PassL | `loudspeaker.ts` : `applyLoudspeakerAnchor` | 확성기(적요"확성기" ∧ 540,000) 지출을 계정 84·과목 86으로 강제 + Pass1 이동 제외(`protectIds`) |
| Pass1 | `fund-realloc.ts` : `reallocateFundSources` | 자금원별 총액(수입−지출) 부족분만 잉여 자금원으로 이동(지출만, 과목 불변). 통장 총잔액 ≥0이면 모든 자금원 최종잔액 ≥0 |
| Pass2 | `item-allocation.ts` : `allocateIncomeToItems` | 자금원 내부에서 수입을 충당 지출의 과목으로 재태깅(지출 과목 불변) |
| Pass3 | `item-balance-zero.ts` : `zeroItemBalances` | 남은 (계정×과목) 불균형을 같은 자금원 잉여 과목 수입 재배정으로 0. 자금원 총액≠0이면 강제 안 함 |
| Pass4 | `schedule-expense-dates.ts` : `scheduleExpenseDates` | 각 (계정×과목) 시트 지출일을 누계 가용액이 덮는 최초일로 뒤로만 이동(금액·계정·과목 불변) |

**두 진입점(같은 코어)**: 직접 계산·표시는 `buildLedgerRows`, .db export·수입지출부 뷰어 materialize는 `adjusted-ledger.ts buildAdjustedAccBook` → `persist-allocation.ts planAllocationPersist`(**내부에서 `buildLedgerRows` 호출**, slice0 원 id 유지 + 이동분 신규 id로 행 생성). 둘이 같은 코어라 **화면==Excel==.db** 보장(parity 회귀: `adjusted-ledger-parity.test.ts`). **불변식**: 원본 `acc_book` 불변, 자금원별 수입합·지출합·통장 총잔액 불변, 지출 과목/금액 불변(계정만 이동), 후보자만.

**Gotchas**:
- **Pass0 gross-up** (caused real user confusion): 음수 수입(흔히 내부 계정이체를 수입 +/−로 기재)을 양수 지출로 전환하면 자금원 **gross 수입이 실제 외부유입보다 부풀려져** 제출보고서에 실수입 초과로 찍힌다. 재배분 코어 버그로 오인 금지 — **데이터(음수수입) 문제**다(통장 총잔액=0이면 진짜 부족도 Pass1 엣지도 아님). `app/scripts/scan-income-gross-up.mjs`로 감지, `diagnose-income-cap.mjs`로 자금원별 수입/지출/통장잔액 진단.
- **Pass3 환급 엣지**: 환급(음수 지출)이 그 과목 수입보다 크면 과목 잔액이 구조적으로 0이 안 될 수 있다(plug 금지 — 경고 표면화 방침).
- **`funding-allocation.ts`(지출내역관리 자금원별 현황)는 재배분 미적용** — 원본 `acc_book`을 그대로 집계(`overspent` 플래그)하므로 재배분 후 산출물과 다를 수 있다. 새 보고집계 화면은 이 SSOT를 거치게 할 것.
...
- **Import** (`POST /api/system/import-sqlite`) restores a `.db` into the selected org. `conflictPolicy`: `overwrite|skip|merge`; `dryRun=true` returns a preview (row counts, organ candidates) with no writes. Write helpers (`bulkInsert`/`bulkUpsert`/`parseConflictPolicy`) are extracted to `import-helpers.ts` (client injected → unit-testable). **BX4 (거래성 테이블은 overwrite 전용)**: CUSTOMER/ACC_BOOK/ACC_BOOK_BAK/ACCBOOKSEND/ESTATE/CUSTOMER_ADDR 는 identity PK 를 strip 하고 fresh insert 하므로 PK 충돌이 원천적으로 없다 → 과거 skip/merge 가 복구 때마다 거래·거래처를 전량 복제했다. 이제 `replaceOrgData = (conflictPolicy==="overwrite")` 로 STEP 4~9 를 가드해 **overwrite 만 거래성 테이블을 교체 import**, skip/merge 는 이들을 건너뛰고 기존 유지(참조코드 CODESET/CODEVALUE/ACC_REL·기관 ORGAN·의견 OPINION 은 자연 PK upsert 라 정책 무관 갱신). backup 페이지 라벨도 "거래 유지(참조·기관정보만 갱신)"로 정정. (overwrite 의 customer/accbooksend 전역삭제 org 스코프화[S4]는 잔여.)
...
- `docs/` — PDCA documentation (plan → design → analysis → report) per feature, archived under `docs/archive/YYYY-MM/` once complete
