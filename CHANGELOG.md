...
- 선관위 제출서류 서식 7 "(예비)후보자 정치자금 수입계정별 회계장부"를 빈 양식 대신 **수입내역 데이터로 자동 채워** 한글파일(.hwpx)로 내려받을 수 있습니다. 서식 7 선택 후 "수입 데이터로 회계장부 생성"을 누르면 입력된 수입내역이 계정·과목별로 정리된 공식 양식이 즉시 생성됩니다.
  - 계정·과목 조합마다 표를 나누고, 그룹 내 일자순 누계·잔액을 자동 계산합니다.
  - 각 행에 연월일·내역·수입금액·누계·잔액·수입제공자(성명·생년월일·주소·직업·전화)·영수증번호가 채워집니다.
