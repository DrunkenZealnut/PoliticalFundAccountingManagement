...
> **Summary**: 수입·지출내역(acc_book, incmSecCd 1·2)을 계정+과목 그룹별로 묶어 공식 form-7(정치자금 수입·지출부) 레이아웃의 표·행을 OWPML 복제로 동적 생성, 그룹 내 수입누계·지출누계·잔액(수입누계−지출누계) 계산된 단일 HWPX로 출력한다.
>
...
## 변경 이력 (v0.4.0.0 후속 — 수입·지출 통합)

> 초기 설계는 "수입 중심, 지출 컬럼 공란"이었으나, form-7은 본래 **정치자금 수입·지출부**이고 사용자 요구에 따라 **지출내역도 같은 수입계정(자금원)별 표에 포함**하도록 변경했다.
>
> - **근거(실데이터 org 9)**: 수입(incm=1)·지출(incm=2)이 **동일 계정·과목 코드**를 공유한다 — 계정 82(보조금)/84(후보자등자산)/85(후원회기부금) × 과목 86(선거비용)/87(선거비용외정치자금). 따라서 `(acc_sec_cd, item_sec_cd)` 그룹에 수입·지출이 자연히 함께 묶인다(별도 매핑 불필요).
> - **조회**: `incm_sec_cd` 필터 제거 → 수입·지출 모두 조회.
> - **그룹 내 행 구성**: 일자순 정렬, 동일자는 수입(incm=1) 먼저. 수입행은 수입 금회·누계만, 지출행은 지출 금회·누계만 채우고, **잔액 = 수입누계 − 지출누계**를 모든 행에 표시(form-7 작성예시 구조와 일치).
> - **템플릿 변경 없음**: `form-7-fill.hwpx`에 이미 `{{지출금회}}`·`{{지출누계}}` 토큰이 존재.
> - 이로써 §7의 "지출 데이터: 범위 외", Open Questions 1·2(잔액 기준·지출 컬럼)가 해소됨.
