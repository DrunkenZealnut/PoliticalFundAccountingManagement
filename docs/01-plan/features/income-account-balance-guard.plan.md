...
> ⚠️ **SUPERSEDED (2026-06-21)** — 자금원 배정 방식의 현재 진실은 [docs/05-reference/자금원배정방식.md](../../05-reference/자금원배정방식.md). 통장 부족 가시화는 **FR-03 표면화**(`detectCandidateShortfalls` + 배너/헤더)로 보고 시점에 구현됨. 본 플랜의 **입력 시점 실시간 가용잔액 UI**는 미구현 보류 상태(별도 입력 UX 작업).
