...
export default function ExpensePage() {
  ...
  async function loadRecords(filters?: SearchFilters | null) {
    ...
    const { data } = await query.order("acc_date").order("acc_sort_num").order("acc_book_id").limit(100000);
    const recs = data || [];
    setRecords(recs);
  ...
  useEffect(() => {
    ...
    Promise.all([
      sb.from("acc_book").select("*, customer:cust_id(name)").eq("org_id", orgId).eq("incm_sec_cd", 2)
        .order("acc_date").order("acc_sort_num").order("acc_book_id").limit(100000),
