...
export default function DashboardLayout({
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      {/* 사이드바 — 데스크탑은 flex 옆, 모바일은 fixed drawer */}
      <aside
        className={`
          fixed inset-y-0 left-0 z-40 w-64 bg-white border-r overflow-y-auto
      ...
      >
        <div className="p-4 border-b">
          <Link
            href="/dashboard"
            onClick={() => setSidebarOpen(false)}
            className="font-bold text-lg hover:text-blue-700 transition-colors"
          >
            정치자금 회계관리
          </Link>
      ...
      {/* 메인 콘텐츠 — min-w-0으로 flex 자식의 최소 너비 잠금 해제 */}
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        {/* 헤더 */}
        <header className="h-14 border-b bg-white flex items-center justify-between px-4 md:px-6 flex-shrink-0">
          {/* 모바일 햄버거 */}
          <button
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <HelpTooltip id="help.toggle">
              <div className="flex items-center gap-2">
                <span className="text-xs md:text-sm text-gray-500 hidden sm:inline">초보자 모드</span>
