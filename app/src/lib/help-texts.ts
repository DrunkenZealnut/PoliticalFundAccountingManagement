...
export const HELP_TEXTS: Record<string, HelpText> = {
  ...
  "expense.balance-guard": { title: "자금원 잔액 가드", description: "입력하려는 거래일 기준으로 자금원(후보자자산·후원회기부금·보조금·보조금외)별 가용잔액과, 이 지출을 저장하면 잔액이 얼마가 되는지(저장 후 예상)를 보여줍니다. 선택한 자금원이 음수가 되면 경고가 뜹니다. 이때 음수는 통장 현금 부족이 아니라 '이 자금원으로 태깅하면 수입지출부·결산에서 음수가 되어 사후 재배분이 필요'하다는 뜻이며, 여유 있는 다른 자금원을 권장합니다. (저장은 막지 않습니다)" },
