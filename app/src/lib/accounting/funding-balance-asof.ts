/* ------------------------------------------------------------------ */
/*  자금원 입력일 기준 시간순 가용잔액 (지출 입력 가드)                    */
/*                                                                    */
/*  후보자 지출 입력 시, 입력하려는 거래일까지의 자금원별 시간순 가용잔액과   */
...
/** 행이 (asOfDate, asOfTime) 시점 이하인가. asOfTime 미지정이면 그 날짜 전체 포함. */
function atOrBefore(row: AsOfRow, asOfDate: string, asOfTime: string | null): boolean {
  if (row.acc_date !== asOfDate) return row.acc_date < asOfDate;
  if (asOfTime == null || asOfTime === "") return true; // 같은 날짜 전체 포함
  return (row.acc_time ?? "") <= asOfTime;
}

/**
 * 자금원별 입력 거래일까지의 시간순 가용잔액.
